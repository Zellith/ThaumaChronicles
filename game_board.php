						...
						if (isset($_SESSION['card_id']))
						{
							...
							if($r['field2']!="NONE")
							{
								...
								if ($r['player_id']!=$_SESSION['p_id']) 
								{
									...
									if($east1 > $west2)
									{
										echo "<script type=text/javascript>"."alert('Player 1 Wins')</script>";
									}
									else
										echo "<script type=text/javascript>"."alert('Player 2 Wins')</script>";
								}
							}
							$i = 0;
							while (isset($p_rec_id[$i])) 
							{
								$sql = "select * from tblplayer_card where rec_id = ".$p_rec_id[$i]." and used != \"DEAD\"";
								$q = mysql_query($sql);
								$r = mysql_fetch_assoc($q);
